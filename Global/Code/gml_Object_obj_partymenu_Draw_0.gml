// Draw a black box over the normal DarkController menu buttons.
// Used as a background for flavour/warning text, similar to the
// ITEM menu. Also persists into the ConfigMenu
...
if (partyMenu == 0 || partyMenu == 1)
{
    ...
    for (i = 0; i < 3; i += 1)
    {
        ...
        // Draw the dot for empty slots, otherwise
        // draw the correct character sprite.abs
        if (global.char[i] == 0)
        {
            draw_text(menu_x + (50 * (i + 1)) + 4, menu_y + 67, ".");
        }
        else
        {
    ...
    // If choosing a slot, draw the SOUL cursor over the correct slot
    draw_set_color(c_white);
    if (partyMenu == 0)
    {
        if (partySelect < 3)
        {
            draw_sprite(spr_heart_harrows, menusiner / 20, menu_x + 42 + (partySelect * 50), menu_y + 48);
        }
        menusiner++;
    }
    ...
    // Divider for choosing a Character
    ...
    // Check if the Character Icons should be dimmed
    // based on which part of the PartyMenu is active.
    ...
    // Configurable Icon Spacing variables
    var iconStart = 39;
    var iconOffset = 71;

    // Different values for Chapter 1 because of Noelle
    if (global.chapter == 1)
    {
        iconStart += 11;
        iconOffset += 20;
    ...
    // Draw the Character Icons in order
    ...
    // Draw the SOUL cursor next to the selected Icon
    if (partyMenu == 1)
    {
        draw_sprite_ext(spr_heart, 0, (menu_x + (charSelect * iconOffset) - iconStart - 15), menu_y + 156, 1, 1, 0, c_white, 1);
    }
    ...
    // Warning-related variables
    // TODO: rework this code so the warning conditions
    // arent being reevaluated EVERY FRAME in the DRAW code.
