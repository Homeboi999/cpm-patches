function scr_jokerdamage()
{
    if (instance_exists(obj_joker))
    {
        var debug_inv = 0;
        if (variable_global_exists("chemg_god_mode"))
        {
            debug_inv = global.chemg_god_mode;
        }
        if (global.inv < 0 && debug_inv == 0)
        {
            scr_damage_cache();
            if (target < 3)
            {
                if (global.hp[obj_joker.hpchar[target]] <= 0)
                {
                    scr_randomtarget();
                    target = mytarget;
                    with (global.charinstance[target])
                    {
                        image_blend = c_white;
                        darkify = 0;
                    }
                }
            }
            chartarget = 3;
            tdamage = damage;
            if (target < 3)
            {
                tdamage = ceil(tdamage - (global.battledf[target] * 3));
                chartarget = obj_joker.hpchar[target];
                if (global.charaction[target] == 10)
                {
                    tdamage = ceil((2 * tdamage) / 3);
                }
                if (tdamage < 1)
                {
                    tdamage = 1;
                }
            }
            if (!instance_exists(obj_shake))
            {
                instance_create(0, 0, obj_shake);
            }
            with (global.charinstance[target])
            {
                hurt = 1;
                hurttimer = 0;
            }
            hpdiff = tdamage;
            with (obj_dmgwriter)
            {
                if (delaytimer >= 1)
                {
                    killactive = 1;
                }
            }
            doomtype = -1;
            with (obj_heart)
            {
                dmgnoise = 1;
            }
            if (target < 3)
            {
                if (global.hp[obj_joker.hpchar[target]] <= 0)
                {
                    doomtype = 4;
                    global.hp[obj_joker.hpchar[target]] -= round(tdamage / 4);
                    hpdiff = round(tdamage / 4);
                }
                else
                {
                    global.hp[obj_joker.hpchar[target]] -= tdamage;
                    if (global.hp[obj_joker.hpchar[target]] <= 0)
                    {
                        hpdiff = abs(global.hp[obj_joker.hpchar[target]] - (global.maxhp[obj_joker.hpchar[target]] / 2));
                        doomtype = 4;
                        global.hp[chartarget] = round(-global.maxhp[obj_joker.hpchar[target]] / 2);
                        scr_dead(target);
                    }
                }
                dmgwriter = instance_create(global.charinstance[target].x, (global.charinstance[target].y + global.charinstance[target].myheight) - 24, obj_dmgwriter);
                dmgwriter.damage = hpdiff;
                dmgwriter.type = doomtype;
            }
            if (target == 3)
            {
                for (hpi = 0; hpi < 3; hpi += 1)
                {
                    chartarget = obj_joker.hpchar[hpi];
                    if (global.hp[chartarget] >= 0)
                    {
                        if (global.charaction[hpi] == 10)
                        {
                            global.hp[chartarget] -= ceil(tdamage / 2);
                        }
                        else
                        {
                            global.hp[chartarget] -= tdamage;
                        }
                        if (global.hp[chartarget] <= 0)
                        {
                            global.hp[chartarget] = round(-global.maxhp[0] / 2);
                        }
                    }
                }
            }
            global.inv = global.invc * 40;
            scr_damage_check();
            gameover = 1;
            if (global.char[0] != 0 && global.hp[obj_joker.hpchar[0]] > 0)
            {
                gameover = 0;
            }
            if (global.char[1] != 0 && global.hp[obj_joker.hpchar[1]] > 0)
            {
                gameover = 0;
            }
            if (global.char[2] != 0 && global.hp[obj_joker.hpchar[2]] > 0)
            {
                gameover = 0;
            }
            if (gameover == 1)
            {
                scr_gameover();
            }
        }
    }
}
